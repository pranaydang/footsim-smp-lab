...
from .surface import foot_surface
from .constants import epiThick
...
class Afferent(object):
    ...
    affid = constants.affid
    affreg = constants.affreg
    affcol = constants.affcol

    def __init__(self,affclass,**args):
        """Initializes an Afferent object.
        Args:
            affclass (string): Afferent class, one of 'SA1', 'SA2','FA1', or 'FA2'.
        Kwargs:
            location (array): Location of the afferent (default: [0,0]).
            noisy (bool): Injects noise into membrane potential (default: False).
        ...
        self.noisy = args.get('noisy',False)
        self.delay = args.get('delay',False)
        self.surface = args.get('surface',foot_surface)
        ...
        self.fit_id = Afferent.affid[self.affclass][self.idx]
        self.fit_reg = Afferent.affreg[self.affclass][self.idx]
    ...
    def affclass(self,affclass):
        try:
            self._affclass = constants.affclass_mapping[affclass]
        except KeyError:
            raise IOError("Afferent class must be one of " + \
                ", ".join(Afferent.affclasses) + ".") from None
...
class AfferentPopulation(object):
    ...
    def __str__(self):
        return 'AfferentPopulation with ' + str(len(self)) + ' afferent(s): ' +\
                str(sum(self.find('SA1'))) + ' SA1, ' + str(sum(self.find('SA2'))) + ' SA2, ' +\
                str(sum(self.find('FA1'))) +\
                 ' FA1, ' + str(sum(self.find('FA2'))) + ' FA2.'
    ...
    @property
    def depth(self):
        reg = self.surface.locate(self.surface.hand2pixel(self.location))
        return np.asarray(list(
            map(lambda x: (x.depth + epiThick(x.surface.locate(x.surface.hand2pixel(x.location)))), self.afferents)))
    ...
    def find(self,affclass):
        ...
        """
        return list(map(lambda x:x.affclass==constants.affclass_mapping[affclass],self.afferents))
    ...
    def response(self,stim):
        ...
        for s in s_iter:
            ...
            with warnings.catch_warnings():
                ...
                spikes, _ = lif_neuron(self,strain,udyn)
                r.append(spikes)
...
class Stimulus(object):
    ...
    def __init__(self,**args):
        ...
        Kwargs:
            ...
            surface (Surface object): The surface on which Stimulus is located
                (default: foot_surface).
        """
    ...
    def compute_profile(self):
        ...
        self._profile, self._profiledyn = skin_touch_profile(
            self.surface,self.trace,self.location,self.fs,self.pin_radius)
        #self._profile, self._profiledyn = skin_touch_profile(
         #   self,self.trace,self.location,self.fs,self.pin_radius)
