...
from .constants import foot_tags,foot_orig,foot_pxl_per_mm,foot_theta,foot_density

default_density = {('SA1',''):10., ('SA2',''):10., ('FA1',''):10.,('FA2',''):10.}
...
class Surface(object):
    ...
    def __init__(self,**args):
        ...
        Kwargs:
            ...
            outline (array): 2D matrix containing outline of surface (and subregions);
                ...
                in array form.
            tags (list): List of region tags (default: all empty strings if outline
                image supplied, otherwise 'AMi').
    ...
    def locate(self,locs):
        ...
        for i in range(0,len(locs)):
            for j in range(0,2):
                locs[i][j]=round(locs[i][j])
        regions = -np.ones((locs.shape[0],),dtype=np.int8)

        if self.outline is None:
            return [self.tags[0] for i in range(0,len(locs))], regions
...
foot_surface = Surface(filename=(os.path.dirname(os.path.dirname(__file__))\
       + '/surfaces/foot.png'),orig=foot_orig,pxl_per_mm=foot_pxl_per_mm,
        theta=foot_theta,density=foot_density,tags=foot_tags)
