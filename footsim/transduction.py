...
from .constants import ihbasis, hardnessRegion
...
def skin_touch_profile(surf,S0,xy,samp_freq,ProbeRad):
    S0 = S0.T # hack, needs to be fixed
    s = S0.shape
    
    region=surf.locate(xy)
    hardness=hardnessRegion(region[0][0])
    E = YoungsModulus(hardness)
    #%we have to convert E from Pa to N/mm^2
    E=E/math.pow(10,6)
    nu = 0.5
...
def skin_touch_profileInsole(surf,S0,xy,samp_freq,ProbeRad,pressure):
    S0 = S0.T # hack, needs to be fixed
    s = S0.shape
...
def YoungsModulus(hardness):

    """ Computes the Young's Modulus based on hardness.

         Arguments:

             hardness(float): Hardness in arbitrary units.
    ...
    H = hardness * sigma / p0
    Yh = (1 + Mi * H) / (math.pow(math.sqrt(1 - H), 3))
    Y = Yh * rho
    E = F0 * (1 - math.pow(poiss, 2)) * Y / (2 * r * p0)

    return E

def find_max_indentation(x):
    """
...
def indentation(**args):

    """ Get indentation based on pressure applied

    Arguments:
        **args:
            region (int) = area of the foot the pin is located. If empty string, require a skin hardness value
            Rsi (int) = radius of the indentation pin, in cm
            pressure (int) = pressure applied using the pin, kPa

    Returns:

        Indentation (float) = depth of indentation

    """
    region = args.get('region', False)
    Rsi = args.get('pin_radius', .25)
    pressure = args.get('pressure')

    if region == False:
        hardness = args.get('hardness')
    else:
        hardness = hardnessRegion(region)  # get hardness of the foot region using Young's modulus

    Eskin = YoungsModulus(hardness) / 1000  # convert Young's modulus from Pa to kPa
    vskin = .48  # Poisson's ratio of skin
    Eindenter = 68000000  # Young's modulus of aluminium (kPa)

    Er = 1 / (((1 - vskin ** 2) / Eskin) + ((1 - .334 ** 2) / Eindenter))  # calculate reduced modulus

    # Er = 1 / (((1 - vskin ** 2) / Eskin) + ((1 - .334 ** 2) / Eindenter)) # calculate reduced modulus

    # indentation = np.sqrt((pressure / (4 / 3 * Er * Rsi ** 0.5) ** 3)) # calculate indentation (mm) using a spherical indetater

    indentation = pressure / 2 / (Eskin * np.pi * Rsi * (
                1 - vskin ** 2))  # calculate indentation using a flat ended cyclindrical indenter

    indentation[indentation > find_max_indentation(hardness)] = find_max_indentation(hardness)
