...
from .surface import Surface,foot_surface

#def plot(obj,**args):
def plot(obj=foot_surface,**args):
...
def plot_stimulus(obj,**args):
    ...
    if not spatial:
        ...
        sur = args.get('surface',foot_surface)
        mid = (bins[1:] + bins[:-1]) / 2.
...
def plot_surface(obj,**args):
    ...
    # show cursor position in foot coordinates (works only in bokeh)
    if locator:
